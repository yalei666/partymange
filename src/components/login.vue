  ...
  <header class="mui-bar mui-bar-nav">
		...
		<div class="mui-content">
			...
			<div class="mui-content-padded">
				<button id='login' class="mui-btn mui-btn-block mui-btn-primary" @click= load>登录</button>
				<div class="link-area">
                 <h5 style="margin-top: 15px;" @click="add">注册账号</h5>
				 <span class="spliter">|</span>
                  <h5 style="margin-top: 15px;">忘记密码</h5>
				</div>
			</div>
	...
	methods:{
		load (){
			axios.post('/users/login',{
				...
				userpassword:parseInt(this.userpassword)
			}).then(response=>{
				let les=response.data;
				if(les.status === '0'){
					mui.toast('登录成功')		
					this.$router.push({name:'home'});
			        this.$store.commit('saveUser',les.result)
...
<style scoped>
h5{display:inline-block;color:#007aff;}
.area {
